...
describe('Hello.tsx', () => {
  ...
  describe('<Hello1 />', () => {
    propsList.forEach((item) => {
      it('[testing-libaray] should render', () => {
        const { getByTestId, getByText } = render(<Hello1 compiler={item.compiler} framework={item.framework} />);
        expect(getByTestId('hello1-h1')).toBeTruthy();
        expect(getByText((content) => content.includes(`${item.compiler}`))).toBeTruthy();
      });
    });
  });
  ...
  describe('<Hello2 />', () => {
    propsList.forEach((item) => {
      it('[testing-libaray] should render', () => {
        const { getByTestId, getByText } = render(<Hello2 compiler={item.compiler} framework={item.framework} />);
